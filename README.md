...
| 檔案 | 尺寸 | 大小 | 用途 |
|---|---|---|---|
| `docs/frames/<date>-<location>-<kind>.jpg` | 160px 寬 | 約 1–3KB | 網站列表顯示、快速查證可疑數值 |
| `docs/frames/full/<date>-<location>-<kind>.jpg` | 320px 寬（**評分用的那一幀**） | 約 16KB | 日後改良判別方法時重跑歷史資料 |

**為什麼縮圖不夠。** 2026-08-16 那筆真火燒雲，拿 160×90 縮圖算暖像素飽和度得到 0.168，
據此判定為「霾」——結論是錯的。縮圖經過降尺度加二次 JPEG 壓縮，色彩統計早被壓平，
和 `camMetrics` 實際評分的 320×180 影格不可互換。在此之前原始影格抓完就丟，
每一次事後回顧都只能用一份失真的替代品。

只存第一幀：`camMetrics` 就是讀它算分的。第二幀唯一的用途是靜止畫面偵測，
而那個結論已經以 `frameDiffMean`／`frameDiffMax` 存進紀錄，不需要留圖。

容量：五場/天 × 16KB ≈ 每天 80KB、每年約 29MB。
