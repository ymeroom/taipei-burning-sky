...
ffmpeg -loglevel error -y -i "$STREAM" -vframes 1 -vf scale=320:-1 -f rawvideo -pix_fmt rgb24 "$FRAME"

BYTES=$(wc -c < "$FRAME")
HEIGHT=$(( BYTES / 320 / 3 ))
[ "$HEIGHT" -gt 0 ] || { echo "抓幀失敗（$BYTES bytes）" >&2; exit 1; }
echo "抓到 320×${HEIGHT} 影格"

node scripts/capture.mjs "$KIND" "$FRAME" 320 "$HEIGHT" ${DATE_ARG:+"$DATE_ARG"}
